...
async def fix_build_error(request: FixRequest):
    """
    Analyse les logs Jenkins et le diff Git fournis directement par l'utilisateur (MODE SIMULATION).
    """
    logger.info(f"--- 🚀 Analyse des logs et diff fournis par l'utilisateur (MODE SIMULATION) ---")
    ...
    try:
        # ═══════════════════════════════════════════════════════════
        # MODE SIMULATION : Appeler les tools MCP avec les données fournies
        # ═══════════════════════════════════════════════════════════

        async with Client(mcp_server_url) as client:
            # 1. Récupérer les logs Jenkins (simulé - données fournies directement)
            ...
            if logs_data.get("error"):
                raise HTTPException(status_code=400, detail=logs_data["error"])

            # 2. Récupérer le diff Git (simulé - données fournies directement)
            ...
            # 3. Construire le prompt formaté à partir des données extraites
            error_lines = logs_data.get("error_lines", [])
            diff_text = diff_data.get("diff", "")
            commit_info = diff_data.get("commit_info", {})

            formatted_prompt = f"""🔥 ERREURS JENKINS BUILD #{request.build_number}
Job: {request.job_name}

{chr(10).join(error_lines[:10])}

📝 COMMIT GITHUB
Commit: {commit_info.get('sha', request.commit_sha)}
...
"""
        ...
        logger.info(f"✅ Données extraites: {len(error_lines)} erreurs, {len(commit_info.get('java_files', []))} fichiers")
...
def read_root():
    ...
    return HTMLResponse(content="""
        ...
        <body>
            <div class="container">
                <h1>🤖 BugFixer Agent (MODE SIMULATION)</h1>
                <p>Fournissez directement les logs Jenkins et les informations du commit pour lancer l'analyse.</p>
                <form id="fixer-form">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                        <div>
                            <label for="job_name">Nom du Job Jenkins</label>
                            <input type="text" id="job_name" name="job_name" value="springboot-user-service" required>
                        </div>
                        <div>
                            <label for="build_number">Numéro de Build</label>
                            <input type="text" id="build_number" name="build_number" value="42" required>
                        </div>
                    </div>
                    <div style="margin-top: 16px;">
                        <label for="repo_owner">Propriétaire du dépôt</label>
                        <input type="text" id="repo_owner" name="repo_owner" value="Safwen707" required>
                    </div>
                    <div style="margin-top: 16px;">
                        <label for="repo_name">Nom du dépôt</label>
                        <input type="text" id="repo_name" name="repo_name" value="springboot-user-service" required>
                    </div>
                    <div style="margin-top: 16px;">
                        <label for="commit_sha">SHA du commit</label>
                        <input type="text" id="commit_sha" name="commit_sha" value="a7f3d91" required>
                    </div>
                    <div style="margin-top: 16px;">
                        <label for="build_logs">Logs Jenkins (complets)</label>
                        <textarea id="build_logs" name="build_logs" rows="8" style="width: 100%; padding: 12px; border: 1px solid #dddfe2; border-radius: 6px; font-family: monospace; font-size: 12px;" required placeholder="Coller les logs Jenkins ici..."></textarea>
                    </div>
                    <div style="margin-top: 16px;">
                        <label for="commit_info">Informations du commit (JSON)</label>
                        <textarea id="commit_info" name="commit_info" rows="12" style="width: 100%; padding: 12px; border: 1px solid #dddfe2; border-radius: 6px; font-family: monospace; font-size: 12px;" required placeholder='{"sha": "...", "author": "...", "date": "...", "message": "...", "java_files": [...]}'></textarea>
                    </div>
            ...
            <script>
                document.getElementById('fixer-form').addEventListener('submit', async function(event) {
                    ...
                    const repoOwner = document.getElementById('repo_owner').value;
                    const repoName = document.getElementById('repo_name').value;
                    const commitSha = document.getElementById('commit_sha').value;
                    const buildLogs = document.getElementById('build_logs').value;
                    const commitInfoText = document.getElementById('commit_info').value;

                    const resultElement = document.getElementById('result');
                    const submitButton = document.getElementById('submit-button');
                    const spinner = document.getElementById('spinner');

                    // Valider le JSON du commit
                    let commitInfo;
                    try {
                        commitInfo = JSON.parse(commitInfoText);
                    } catch (e) {
                        resultElement.textContent = "Erreur : Le JSON des informations de commit est invalide. Vérifiez la syntaxe.";
